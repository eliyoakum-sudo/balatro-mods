...
extern PRECISION vec2 definitive;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    number fac = 0.8 + 0.9*sin(11.*uv.x+4.32*uv.y + definitive.r*12. + cos(definitive.r*5.3 + uv.y*4.2 - uv.x*4.));
    
    vec2 rotater = vec2(cos(definitive.r*0.1221), sin(definitive.r*0.3512));
    
    number angle = dot(rotater, adjusted_uv)/(length(rotater)*length(adjusted_uv));
    
    number fac2 = max(min(5.*cos(definitive.g*0.3 + angle*3.14*(2.2+0.9*sin(definitive.r*1.65 + 0.2*definitive.g))) - 4. - max(2.-length(20.*adjusted_uv), 0.), 1.), 0.);
    number fac3 = 0.5 + 0.5*sin(8.*uv.x+2.32*uv.y + definitive.r*5. - cos(definitive.r*2.3 + uv.x*8.2));
    number fac4 = 0.5 + 0.5*sin(3.*uv.x+2.32*uv.y + definitive.r*8.111 + sin(definitive.r*1.3 + uv.y*11.2));

    number maxfac = max(max(fac, max(fac2, max(fac3, max(fac4, 0.0)))) + 0.5*(fac+fac2+fac3+fac4), 0.);
