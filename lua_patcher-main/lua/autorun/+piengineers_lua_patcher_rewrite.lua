...
LUA_PATCHER.VERSION = "4.2.2"
LUA_PATCHER.VERSION_DATE = "2025-11-12"
local Log, LogError
if gmod then
  local initialized
  local ConVarLogging = CreateConVar("lua_patcher_logging", "1", FCVAR_ARCHIVE, "Logs errors prevented by Lua Patcher.")
  ...
  LogError = function(...)
    if not LUA_PATCHER.convars_initialized or ConVarLogging:GetBool() then
...
if gmod then
  local ConVarEnable = CreateConVar("lua_patcher_enable", "1", FCVAR_ARCHIVE, "Enables Lua Patcher.")
  local CheckPatch
  CheckPatch = function()
